...
int get_next_byte(void) // doc 1 ky tu
{
  if (pos == rawtext_size) {
    ++pos;
    return (PLL_EOS);
...
int get_next_symbol(void) // phan loai ky tu
{
  int ch, sym;
  ...
  if (ch == PLL_EOS)
    return (PLL_SYM_EOF);
  if (ch >= PLL_ASCII_SIZE)
    return (PLL_SYM_UNKNOWN);
...
pllLexToken get_token(int *input) // gom ky tu thanh token
{
  pllLexToken token;
  int start_pos, isFloating = 0;
    ...
    if (*input != PLL_SYM_CHAR) {
      ...
      if (rawtext[pos - 1] == 'E' || rawtext[pos - 1] == 'e') {
        *input = get_next_symbol();
        ...
        if (*input == PLL_SYM_PLUS || *input == PLL_SYM_DASH ||
            *input == PLL_SYM_DIGIT) {
          do {
            *input = get_next_symbol();
          } while (*input == PLL_SYM_DIGIT);
      ...
      if (*input == PLL_SYM_CHAR) {
        do {
          *input = get_next_symbol();
        } while (*input == PLL_SYM_CHAR || *input == PLL_SYM_DIGIT ||
                 *input == PLL_SYM_DOT);
        token.len = pos - start_pos;
        token.tokenType = PLL_TOKEN_STRING;
      }
  ...
  case PLL_SYM_CHAR:
    do {
      *input = get_next_symbol();
             ...
             *input == PLL_SYM_DASH || *input == PLL_SYM_DOT);
    token.len = pos - start_pos;
    token.tokenType = PLL_TOKEN_STRING;
    if (*input == PLL_SYM_LFCR)
      --token.len;
  ...
  case PLL_SYM_LFCR:
    do {
      *input = get_next_symbol();
    } while (*input == PLL_SYM_CR || *input == PLL_SYM_LFCR ||
             *input == PLL_SYM_LF);
...
void init_lexan(const char *text, long n) // reset lexer
{
  rawtext = text;
  rawtext_size = n;
  pos = 0;
